...
document.addEventListener(pimcore.events.postOpenObject, (e) => {
    ...
    {
        fetch('/admin/stf-trans/translator2/object-can-translate/' + id, {
            method: 'POST',
            headers: myHeaders
        })
        ...
        .then(result => {
            if (result.length > 0) {
                initTranslateObjectButton(result);
            }
        })
        .catch(error => {
        });
    ...
    function initTranslateObjectButton(translateFields)
    {
                ...
                var buttonConfig = {
                    ...
                    handler: function (button) {
                        ...
                        window.setTimeout(function() {
                            var translateData = {};
                            var langNeedTransTotal = translateFields.length;

                            for (const translateField of translateFields) {
                                translateData[translateField] = {};
                                fetch('/admin/stf-trans/translator2/object-field/' + id + '/' + button.config.language + '/' + translateField, requestOptions)
                                    .then(response => response.json())
                                    .then(result => {
                                        translateData[translateField][result.language] = result.value;

                                        langNeedTransTotal--;
                                        if (langNeedTransTotal == 0) {
                                            var mergeRequestOptions = requestOptions;
                                            mergeRequestOptions.body = JSON.stringify(translateData);
                                            fetch('/admin/stf-trans/translator2/object-merge/' + id , mergeRequestOptions)
                                                .then(response => response.text())
                                                .then(result => {
                                                    pimcore.helpers.loadingHide();
                                                    object.reload([]);
        ...
        e.detail.object.toolbar.add({
            ...
            handler: function (button) {
                ...
                window.setTimeout(function() {
                    var translateData = {};
                    var langNeedTransTotal = (websiteLanguages.length - 1) * translateFields.length;
                    ...
                    for (const translateField of translateFields) {
                        ...
                        for (let i = 0; i < websiteLanguages.length; i++) {
                            var lang = websiteLanguages[i];
                            if (lang != 'vi') {
                                fetch('/admin/stf-trans/translator2/object-field/' + id + '/' + lang + '/' + translateField, requestOptions)
                                    .then(response => response.json())
                                    .then(result => {
                                        translateData[translateField][result.language] = result.value;
